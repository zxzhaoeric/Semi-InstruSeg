...
def main(args):
    ...
    model_save_dir = '_'.join([model_save_dir, str(args.jaccard_weight), \
         str(args.batch_size), str(args.input_height), str(args.input_width), str(0.2), str('interp')])
    ...
    for fold in args.folds:
        ...
        best_epoch, best_record = sorted(metrics_records_fold.items(), key=lambda item: item[1]['miou'], reverse=True)[0]
        ...
        # iou_list.append(best_record['miou'])
        # dice_list.append(best_record['mdice'])
        # print fold results
    ...
    # mean_metrics['std_miou'] = std_iou
    # mean_metrics['std_mdice'] = std_dice
    # print mean results
...
def train_fold(fold, args):
    ...
    def trainer_start_callback(engine):
        ...
        if args.resume:
            ...
            model.load_state_dict(torch.load('../model_ckpt/MT/UNet11_binary_0.0001_0.3_16_512_640/fold_0_iter_6000.pth'), strict=False)
...
if __name__ == '__main__':
    ...
    # dirs
    parser.add_argument('--train_dir', type=str, default='../data/aug0.33',
