...
mask_folder_sepconv = {
    'binary': 'binary_masks_sepconv',
    'parts': 'parts_masks_sepconv',
    'instruments': 'instruments_masks_sepconv'
}

mask_folder_rgb = {
    'binary': 'binary_masks_rgb',
    'parts': 'parts_masks_rgb',
    'instruments': 'instruments_masks_rgb'
}
mask_folder_aug = {
    'binary': 'binary_masks_aug',
    'parts': 'parts_masks_aug',
    'instruments': 'instruments_masks_aug'
}
"""
file utilities:
...
def trainval_split(data_dir, fold):
    ...
    for idx in range(1, 9):
        ...
        v_file_names = (Path(data_dir.replace('interp0.11_new', 'cropped_train')) / ('instrument_dataset_' + str(idx)) / 'images').glob('*')
        v_file_names = list(sorted(v_file_names))
        file_tmp = []

        sep = int(5)
        file_tmp = filenames[::sep]
        ...
        if idx in folds[fold]:
            val_file_names += v_file_names
        else:
            train_file_names += file_tmp
            full_train += filenames

    return full_train, val_file_names
