...
import math
from datasets.frame_loader3 import FrameLoader, trainval_split
...
parser.add_argument('--model', metavar='MODEL', default='CycleNet', choices=model_names, help='model name')
...
parser.add_argument('--torch_home', default='./.torch', type=str, metavar='TORCH_HOME',
...
parser.add_argument('--lr_milestones', type=int, nargs='+',
                    default=[250, 450], help="Spatial dimension to " + "crop training samples for training")
...
parser.add_argument('--save_freq', type=int, default=10, metavar="SAVE_FREQ",
                    help='frequency of saving intermediate models (default: 20)')

parser.add_argument('--epochs', default=1000, type=int, metavar="EPOCHES",
                    help='number of total epochs to run (default: 500)')

# Training sequence, supports a single sequence for now
parser.add_argument('--train_file', metavar="TRAINING_FILE", help='training file')
parser.add_argument('--sequence_length', default=2, type=int, metavar="SEQUENCE_LENGTH", help='number of interpolated frames (default : 7)')
...
parser.add_argument('--train_n_batches', default=-1, type=int, metavar="TRAIN_N_BATCHES",
                    help="Limit the number of minibatch iterations per epoch. Used for debugging purposes. (default : -1")
...
def get_train_and_valid_data_loaders(block, args):
    ...
    train_filenames, valid_filenames = trainval_split(args.train_file)
    train_dataset = FrameLoader(args, filename=train_filenames, is_training=True, back_propagation = False)
    ...
    train_loader = torch.utils.data.DataLoader(train_dataset, sampler=train_sampler,shuffle=(train_sampler is None), **tkwargs)

    block.log('Number of Training Images: {}:{}'.format(len(train_loader.dataset), len(train_loader)))

    # validation dataloader
    vkwargs = {'batch_size': args.val_batch_size,
               'num_workers': args.workers,
               'pin_memory': False, 'drop_last': True}
    val_dataset = FrameLoader(args, filename=valid_filenames)
    val_loader = torch.utils.data.DataLoader(val_dataset, shuffle=False, **vkwargs)

    block.log('Number of Validation Images: {}:{}'.format(len(val_loader.dataset), len(val_loader)))
...
def build_and_initialize_model_and_optimizer(block, args):
    ...
    optimizer = args.optimizer_class([p for p in model.parameters() if p.requires_grad], lr=args.lr)
...
def train_step(batch_cpu, model, optimizer, block, args, print_linf_grad=False):
    ...
    # if print_linf_grad:
    #     block.log('graldients Linf: {:0.3f}'.format(calc_linf_grad_norm(model.parameters())))
...
def evaluate(model, val_loader, block, args, epoch):
    ...
    with torch.no_grad():
        ...
        num_batches = 10
        # num_batches = len(val_loader)-args.sequence_length
        ...
        for i, batch in enumerate(tqdm(val_loader, total=num_batches)):
            ...
            for b in range(args.val_batch_size):
                ...
                if args.write_images:
                    image_filename =  input_filenames[b].split('/')[-1]
                    path = os.path.join( args.save_root, image_filename.split('.')[0]+'_0.5.png' )
...
def train_epoch(epoch, args, model, optimizer, lr_scheduler,
    ...
    for i, batch in enumerate(tqdm(train_loader, total=num_batches)):
        ...
        loss, outputs, _ = train_step(
            inputs, model, optimizer, block, args,
...
def main():
    ...
    with tools.TimerBlock("Building {} Model and {} Optimizer".format(args.model, args.optimizer_class.__name__)) as block:
        model, optimizer = build_and_initialize_model_and_optimizer(block, args)

    # Learning rate scheduler.
    with tools.TimerBlock("Building {} Learning Rate Scheduler".format(args.optimizer)) as block:
