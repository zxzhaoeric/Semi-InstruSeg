...
import torchvision
import sys
from models.model_utils import conv2d, deconv2d, gram, gram_ppretrained_model'
...
import cv2
import numpy as np
sys.path.append("../semi-seg/")
from visualize.flow_vis import flow_to_color
...
class SDCNet2D(nn.Module):
    ...
    def network_output(self, input_images, input_flows):
        ...
        input_flows = [(input_flow - self.flow_mean) / (3 * self.flow_std) for input_flow in input_flows]
        ...
        rgb_mean = concated_images.view(concated_images.size()[:2] + (-1,)).mean(dim=-1).view(concated_images.size()[:2] + 2 * (1,))
    ...
    def forward(self, input_dict, label_image=None):
        ...
        flow_arrow = self.flow_to_arrow(flow_prediction)
        flow_color = flow_to_color(flow_prediction)

        image_prediction = self.warp_bilinear(last_image, flow_prediction)

        pred_features = self.vgg(image_prediction/ self.rgb_max)
        target_features = self.vgg(target_image/ self.rgb_max)
        ...
        losses['color_gradient'] = self.L1Loss(torch.abs(image_prediction[..., 1:] - image_prediction[..., :-1]), torch.abs(target_image[..., 1:] - target_image[..., :-1])) + \
                                   self.L1Loss(torch.abs(image_prediction[..., 1:, :] - image_prediction[..., :-1, :]), torch.abs(target_image[..., 1:, :] - target_image[..., :-1, :]))

        losses['flow_smoothness'] = self.L1Loss(flow_prediction[..., 1:], flow_prediction[..., :-1]) + self.L1Loss(flow_prediction[..., 1:, :], flow_prediction[..., :-1, :])

        #style loss
        pred = [gram(fmap) for fmap in pred_features]
        gt = [gram(fmap) for fmap in target_features]
        losses['style'] = self.L1Loss(pred[0], gt[0])
        for i in range(1, 4):
            losses['style'] = losses['style'] + self.L1Loss(pred[i], gt[i])
        # # perceptual loss
        # pred_p = [gram_p(fmap) for fmap in pred_features]
        # gt_p = [gram_p(fmap) for fmap in target_features]
        # losses['perceptual'] = self.L1Loss(pred_p[0], gt_p[0])
        # for i in range(1, 4):
        #     losses['perceptual'] = losses['perceptual'] + self.L1Loss(pred_p[i], gt_p[i])
        #losses['tot'] = 0.7 * losses['color'] + 0.2 * losses['color_gradient'] + 0.1 * losses['flow_smoothness']
        #losses['tot'] = 1.0 * losses['color'] + 0.2 * losses['flow_smoothness'] + 0.1 * losses['color_gradient']+  1000*losses['style']
        losses['tot'] = 2.0*losses['color'] + 1000* losses['style']+ 0.2 * losses['flow_smoothness'] + 0.1 * losses['color_gradient']
        if label_image is not None:
            image_prediction = label_prediction

        return losses, image_prediction, target_image, flow_color
