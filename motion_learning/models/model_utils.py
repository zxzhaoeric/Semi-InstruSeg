...
def deconv2d(channels_in, channels_out, kernel_size=4, stride=2, padding=1, bias=True):
    return nn.Sequential(
        ...
        nn.Conv2d(channels_in, channels_out, kernel_size=3, stride=1,padding=1, bias=bias),
...
def getFlowCoeff():
    ...
    C01 = t * t
    C10 = (1 - t) * (1 - t)
    return torch.FloatTensor([C00])[None, None, None, :].permute(3, 0, 1, 2).cuda(), torch.FloatTensor([C01])[None, None, None,:].permute(3, 0, 1, 2).cuda(), \
           torch.FloatTensor([C10])[None, None, None, :].permute(3, 0, 1, 2).cuda(), torch.FloatTensor([C11])[None, None,None, :].permute(3, 0, 1, 2).cuda()
